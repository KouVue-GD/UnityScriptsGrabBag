...
public class Movement : MonoBehaviour
{
    [Serializable]
    enum TypeOfGame{
        firstPerson, thirdPerson, topdown, sidescroller
    }
    ...
    public Rigidbody rb;
    void Start(){
        if(rb == null && (typeOfGame == TypeOfGame.firstPerson || typeOfGame == TypeOfGame.thirdPerson || typeOfGame == TypeOfGame.topdown) ){
            rb = GetComponent<Rigidbody>();    
        }

        if(rb2d == null && (typeOfGame == TypeOfGame.topdown || typeOfGame == TypeOfGame.sidescroller)){
    ...
    public void LimitSpeed(float passedMaxSpeed){
        //limit speed
        if(rb.velocity.sqrMagnitude > passedMaxSpeed){
            rb.velocity = Vector3.ClampMagnitude(rb.velocity, passedMaxSpeed);
        }
    }

    public void LimitSpeedSideScroller(float passedMaxSpeed){
        //limit speed
        if(rb2d.velocity.sqrMagnitude > passedMaxSpeed){
            rb2d.velocity = Vector3.ClampMagnitude(rb2d.velocity, passedMaxSpeed);
        }
    }
    ...
    public void Slowdown(float horizontal, float vertical){
        //slowdown
        if(horizontal == 0){
        ...
        if(vertical == 0){
            rb.velocity = new Vector3(rb.velocity.x, rb.velocity.y, rb.velocity.z * slowdownFactor);
        }
    }

    public void SlowdownSideScroller(float horizontal){
        //slowdown
        if(horizontal == 0){
            rb2d.velocity = new Vector3(rb.velocity.x * slowdownFactor, rb.velocity.y, rb.velocity.z);
        }
    }
